        ...
        for electrode in megapwrspdf[megapwrspdf['resampled'] == True]['electrode'].unique():
            for stim in megapwrspdf['stim'].unique():
                ...
                if (len(tolook) > 0) & (len(tolook1) > 0) & (len(tolook2)> 0):
                    #vals.append([electrode, stim, avpwripsi/avpwrcontra, tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])  #REMOVED
                    vals.append([electrode, stim, 'ipsi',   avpwripsi,   tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])   #NEW
                    vals.append([electrode, stim, 'contra', avpwrcontra, tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])   #NEW
            
    valsdf = pd.DataFrame(vals, columns = ['electrode', 'stim', 'hemi', 'value', 'plotpos', 'plotlabel'])             #NEW: changed ipsi/contra simply to hemi, and added value (to make it 6-item list)
    for hemi in ['ipsi', 'contra']:                                                                                   #MODIFIED
        fig, ax = plt.subplots()

        v = valsdf[valsdf['hemi'] == hemi]

        bp1 = v[v['stim'] == 'C']
        bp2 = v[v['stim'] == 'S']
        
        
        box1 = ax.boxplot(bp1.groupby(['plotpos'])['value'].apply(list), positions = np.array(sorted(bp1['plotpos'].unique()))-0.3, widths = 0.5, showfliers=False)
        bpcol(box1, colourscontrol[1])
        ax.scatter(rand_jitter(bp1['plotpos']) - 0.3, bp1['value'], color = colourscontrol[1], alpha = 0.6)
        
        box2 = ax.boxplot(bp2.groupby(['plotpos'])['value'].apply(list), positions = np.array(sorted(bp2['plotpos'].unique()))+0.3, widths = 0.5, showfliers=False)
        bpcol(box2, coloursstim[1])
        ax.scatter(rand_jitter(bp2['plotpos'])+ 0.3, bp2['value'], color = coloursstim[1], alpha = 0.6)
        ...
        #ax.set_ylabel('Ipsilesional/Contralesional mean power', fontsize = fonti)          #REMOVED
        ax.set_ylabel(f'{hemi} mean power', fontsize=fonti)                                 #NEW
        #ax.set_title(powspecbands[freq] + ' frequency band', fontsize = fonti)             #REMOVED
        ax.set_title(powspecbands[freq] + f' band (all channels, {hemi})', fontsize=fonti)  #NEW
        ...
        #plt.savefig('/home/achanna/projects/rrg-bojana/achanna/kylie_edits/resampled_plots/' +powspecbands[freq] + '_frac_nice_dropmice.png', bbox_inches='tight') #MODIFIED
        plt.savefig(outdir + powspecbands[freq] + f'_{hemi}_allchannels_resampled_dropmice.png', bbox_inches='tight')                                                         #NEW
            ...
            for electrode in megapacdf[megapacdf['resampled'] == True]['electrode'].unique():
                for stim in megapacdf['stim'].unique():
                    ...
                    if len(tolook) > 0:
                        ...
                        #vals.append([electrode, stim, avMIipsi/avMIcontra,tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])     #REMOVED
                        vals.append([electrode, stim, 'ipsi',   avMIipsi,   tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])    #NEW
                        vals.append([electrode, stim, 'contra', avMIcontra, tolook['plotpos'].unique()[0], tolook['plotlabel'].unique()[0]])    #NEW
                
        valsdf = pd.DataFrame(vals, columns = ['electrode', 'stim', 'hemi', 'value', 'plotpos', 'plotlabel'])                                   #NEW: changed ipsi/contra simply to hemi
        
        for hemi in ['ipsi', 'contra']:         #IMPROVED
            fig, ax = plt.subplots()
            ...
            box1 = ax.boxplot(bp1.groupby(['plotpos'])['value'].apply(list), positions = np.array(sorted(bp1['plotpos'].unique()))-0.15, widths = 0.3, showfliers=False)
            bpcol(box1, colourscontrol[1])
            ax.scatter(rand_jitter(bp1['plotpos']) - 0.15, bp1['value'], color = colourscontrol[1], alpha = 0.6)
            
            box2 = ax.boxplot(bp2.groupby(['plotpos'])['value'].apply(list), positions = np.array(sorted(bp2['plotpos'].unique()))+0.15, widths = 0.3, showfliers=False)
            bpcol(box2, coloursstim[1])
            ax.scatter(rand_jitter(bp2['plotpos'])+ 0.15, bp2['value'], color = coloursstim[1], alpha = 0.6)
            ...
            ax.xaxis.set_tick_params(labelsize=fonti-2)
            ax.set_xlabel('Days post stroke', fontsize = fonti)
            #ax.set_ylabel('Ipsilesional/Contralesional MI', fontsize = fonti)                          #REMOVED
            ax.set_ylabel(f'{hemi} MI', fontsize=fonti)                                                 #NEW

            #ax.set_title(lows[low] + highs[high] + ' coupling', fontsize = fonti)                      #REMOVED
            ax.set_title(lows[low] + highs[high] + f' coupling (all channels, {hemi})', fontsize=fonti) #NEW
            ...
            #plt.savefig('/home/achanna/projects/rrg-bojana/achanna/kylie_edits/resampled_plots/' + lowlows[low] + highhighs[high] + '_frac_nice_scaled_dropmice.png', bbox_inches='tight') #REMOVED
            plt.savefig(outdir + lowlows[low] + highhighs[high] + f'_{hemi}_allchannels_resampled_dropmice.png', bbox_inches='tight')                                                                 #NEW
