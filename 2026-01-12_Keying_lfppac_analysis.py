...
# Changes made from Monica's code: dropping channels 0&15, commented out unresampled data proessing as it is not necessary to rerun
...
directories = sorted(glob.glob('/home/achanna/projects/rrg-bojana/bellvila/Kylie/Keying/202*/'), reverse = True) #UPDATED THIS
...
for directory in directories:
    #print(directory)
    subdirectories = sorted(glob.glob(directory + 'Monica_Kylie_*/*kyliemouse-*')) #I CHANGED IT BACK TO KYLIEMOUSE
    ...
    for subdirectory in subdirectories:
        ...
        if run == 1: #continue only if data loaded
            ...
            LFP = signal.filtfilt(b, a, rawclip)                        #I UNCOMMENTED THIS FOR UNRESAMPLED (JAN 13)
            ...
            if dec > 1:
                ...
                for i in range(len(LFP)):                                                                           #I UNCOMMENTED THIS BLOCK FOR UNRESAMPLED (JAN 13)
            ...
            for datatype, LFPdata, resampled in [("unresampled", LFP, False),("resampled", LFPresampled, True)]: #I ADDED THIS AND CHANGED STUFF BELOW IT TO BE MORE GENERAL TO BOTH
                #slicing data into windows
                for i in range(int(np.floor(len(LFPdata[0])/(totdur*Fs)))) :
    #                tolook1 = LFP[:, int(totdur*Fs*i):int(totdur*Fs*i + totdur*Fs)]
                    tolook2 = LFPdata[:, int(totdur*Fs*i):int(totdur*Fs*i + totdur*Fs)]
    ...
    #                    pwrspdata.append([date, subtime, electrode, totdur, i, pwrspwin, Fs, freq, pwrsp, False])
                    ...
                    for electrode in range(len(tolook2)):  
                        ...
                        pwrspdata.append([date, subtime, electrode+1, totdur, i, pwrspwin, Fs, freq, pwrsp, resampled])
    ...
    #                            pacdata.append([date, subtime, electrode, totdur, i,pacwindow,pacwin, Fs, 'tort', False,  p, xpac, pval, xpac_smean])
                        ...
                        for electrode in range(len(tolook2)):
                            for pacwin in range(int(np.floor(len(tolook2[0])/(pacwindow*Fs)))):
                                ...
                                pval = p.infer_pvalues(p=0.05)        
                                xpac_smean = xpac[pval < .05].mean()  
                                pacdata.append([date, subtime, electrode+1, totdur, i,pacwindow, pacwin, Fs, 'tort', resampled,  p, xpac, pval, xpac_smean])
                                # date, subtime, electrode, total duration analyzed, total duration window, window over which pac is calculated
                                # sampling frequency,is it resampled?, pacmethod, pacresults, pacresults2, where pvals are significant, mean pval
...
#seperating outputs so we can save the resampled/unresampled PAC and Pwrspec into different pickle files
    pwrsp_unrs = [row for row in pwrspdata if row[-1] == False]
    pwrsp_rs = [row for row in pwrspdata if row[-1] == True]
    pac_unrs = [row for row in pacdata if row[9] == False]
    pac_rs = [row for row in pacdata if row[9] == True]

#saving the unresampled output in my own folders (Pickling)
    with open("/lustre06/project/6061907/achanna/keying_edits/unresampled/" + date + "_unresampledLFP_pwrsp_dec.pickle", "wb") as fp:
        pickle.dump(pwrsp_unrs, fp)
    with open("/lustre06/project/6061907/achanna/keying_edits/unresampled/" + date + "_unresampledLFP_pac_dec.pickle", "wb") as fp:
        pickle.dump(pac_unrs, fp)

#saving the resampled output in my own folders (Pickling)
    with open("/lustre06/project/6061907/achanna/keying_edits/resampled/" + date + "_resampledLFP_pwrsp_dec.pickle", "wb") as fp:         
        pickle.dump(pwrsp_rs, fp)
    with open("/lustre06/project/6061907/achanna/keying_edits/resampled/" + date + "_resampledLFP_pac_dec.pickle", "wb") as fp:
        pickle.dump(pac_rs, fp)
