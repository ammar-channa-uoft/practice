...
megapwrspdf = pd.read_pickle("/home/achanna/projects/rrg-bojana/achanna/megapwrspdf_unresampled.pkl")
megapacdf   = pd.read_pickle("/home/achanna/projects/rrg-bojana/achanna/megapacdf_unresampled.pkl")
...
outdir = "/home/achanna/projects/rrg-bojana/achanna/kylie_edits/unresampled_plots/"
...
for date in megapwrspdf['date'].unique():
    ...
    plt.savefig(outdir +date + '_powspec_dropmice.png', bbox_inches='tight')
...
for freq in range(len(powspecfrqs)-1):
    ...
    for hemi in ['ipsi', 'contra']:                                                                                   #MODIFIED
        ...
        ax.set_ylabel(f'{hemi} mean power', fontsize=fonti)
        ax.set_title(powspecbands[freq] + f' band (all channels, {hemi})', fontsize=fonti)
        ...
        plt.savefig(outdir + powspecbands[freq] + f'_{hemi}_allchannels_dropmice.png', bbox_inches='tight')
        plotcounter += 1
    ...
    plt.savefig(outdir +powspecbands[freq] + '_frac_nice_dropmice.png', bbox_inches='tight')
...
for freq in range(len(powspecfrqs)-1):
    ...
    plt.savefig(outdir +powspecbands[freq] + '_frac_mouseavg_dropmice.png', bbox_inches='tight')
...
for low in range(len(lows)):
    for high in range(len(highs)):
        ...
        for hemi in ['ipsi', 'contra']:         #IMPROVED
            ...
            ax.xaxis.set_tick_params(labelsize=fonti-2)
            ax.set_xlabel('Days post stroke', fontsize=fonti)
            ax.set_ylabel(f'{hemi} MI', fontsize=fonti)
            ax.set_title(lows[low] + highs[high] + f' coupling (all channels, {hemi})', fontsize=fonti)
            ...
            plt.savefig(outdir + lowlows[low] + highhighs[high] + f'_{hemi}_allchannels_dropmice.png', bbox_inches='tight')
            plotcounter += 1                    #MOVED HERE
        ...
        plt.savefig(outdir + lowlows[low] + highhighs[high] + '_frac_nice_scaled_dropmice.png', bbox_inches='tight')
