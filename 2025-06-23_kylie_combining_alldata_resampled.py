...
# COMBINING CODE (this one is for resampled data)
...
##NEW: 2024 resampled that were saved seperately
pwrsp_2024_resampled = sorted(glob.glob("/home/achanna/projects/rrg-bojana/achanna/kylie_edits/resampled_new/*_resampledLFP_pwrsp_dec.pickle"))
pac_2024_resampled = sorted(glob.glob("/home/achanna/projects/rrg-bojana/achanna/kylie_edits/resampled_new/*_resampledLFP_pac_dec.pickle"))
...
#NEW:drop old 2024 resampled rows so resampled_new replaces them
old2024_pwr = megapwrspdf['date'].astype(str).str.startswith('2024')
old2024_pac = megapacdf['date'].astype(str).str.startswith('2024')

megapwrspdf = megapwrspdf[~(old2024_pwr & (megapwrspdf['resampled'] == True))].reset_index(drop=True)
megapacdf   = megapacdf[~(old2024_pac & (megapacdf['resampled'] == True))].reset_index(drop=True)

#NEW
for pwr_file in pwrsp_2024_resampled:
    pac_file = pwr_file.replace("_pwrsp_dec.pickle", "_pac_dec.pickle")

    with open(pwr_file, "rb") as f:
        pwrspvals = pickle.load(f)
...
megapwrspdf = megapwrspdf[megapwrspdf['resampled'] == True].reset_index(drop=True)
megapacdf   = megapacdf[megapacdf['resampled'] == True].reset_index(drop=True)
...
megapwrspdf.to_pickle("/home/achanna/projects/rrg-bojana/achanna/megapwrspdf_resampled.pkl")
megapacdf.to_pickle("/home/achanna/projects/rrg-bojana/achanna/megapacdf_resampled.pkl")
